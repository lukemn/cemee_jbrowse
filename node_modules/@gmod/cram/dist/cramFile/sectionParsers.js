...
var _require = require('../binary-parser'),
    Parser = _require.Parser;

var singleItf8 = new Parser().itf8();

var cramFileDefinition = {
  parser: new Parser().string('magic', { length: 4 }).uint8('majorVersion').uint8('minorVersion').string('fileId', { length: 20, stripNull: true }),
  maxLength: 26
};

var cramBlockHeader = {
  parser: new Parser().uint8('compressionMethod', {
    formatter: /* istanbul ignore next */function formatter(b) {
      var method = ['raw', 'gzip', 'bzip2', 'lzma', 'rans'][b];
      if (!method) throw new Error('compression method number ' + b + ' not implemented');
      return method;
    }
  }).uint8('contentType', {
    formatter: /* istanbul ignore next */function formatter(b) {
...
};var cramTagDictionary = new Parser().itf8('size').buffer('entries', {
  length: 'size',
  formatter: /* istanbul ignore next */function formatter(buffer) {
...
var parseByteAsBool = new Parser().uint8(null, {
  formatter: /* istanbul ignore next */function formatter(val) {
    return !!val;
  }
});

var cramPreservationMap = new Parser().itf8('mapSize').itf8('mapCount').array('entries', {
  length: 'mapCount',
  type: new Parser().string('key', {
    ...
    choices: {
      ...
      TD: new Parser().nest(null, {
        ...
        formatter: /* istanbul ignore next */function formatter(data) {
          return data.entries;
        }
      })
    }
...
function formatMap(data) {
  var map = {};
  data.entries.forEach(function (_ref) {
    var key = _ref.key,
        value = _ref.value;
...
var unversionedParsers = {
  ...
  cramEncoding: function cramEncoding(majorVersion) {
    var parser = new Parser().namely('cramEncoding').itf8('codecId').itf8('parametersBytes').choice('parameters', {
      ...
      choices: {
        ...
        // HUFFMAN_INT
        3: Parser.start().itf8('numCodes').array('symbols', { length: 'numCodes', type: singleItf8 }).itf8('numLengths').array('bitLengths', { length: 'numLengths', type: singleItf8 }),
        4: Parser.start() // BYTE_ARRAY_LEN
        .nest('lengthsEncoding', { type: 'cramEncoding' }).nest('valuesEncoding', { type: 'cramEncoding' }),
  ...
  cramDataSeriesEncodingMap: function cramDataSeriesEncodingMap(majorVersion) {
    return new Parser().itf8('mapSize').itf8('mapCount').array('entries', {
      length: 'mapCount',
      type: new Parser().string('key', { length: 2, stripNull: false }).nest('value', { type: this.cramEncoding(majorVersion).parser })
    });
  },
  cramTagEncodingMap: function cramTagEncodingMap(majorVersion) {
    return new Parser().itf8('mapSize').itf8('mapCount').array('entries', {
      ...
      type: new Parser().itf8('key', {
        formatter: /* istanbul ignore next */function formatter(integerRepresentation) {
          return (
            /* istanbul ignore next */
...
function getSectionParsers(majorVersion) {
  var parsers = (0, _assign2.default)({}, unversionedParsers);
  (0, _keys2.default)(versionedParsers).forEach(function (parserName) {
    parsers[parserName] = versionedParsers[parserName](majorVersion);
  });
  return parsers;
}

module.exports = { cramFileDefinition: cramFileDefinition, getSectionParsers: getSectionParsers };