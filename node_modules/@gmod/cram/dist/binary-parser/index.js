...
var NAME_MAP = {};
(0, _keys2.default)(PRIMITIVE_TYPES).concat((0, _keys2.default)(SPECIAL_TYPES)).forEach(function (type) {
...
(0, _keys2.default)(PRIMITIVE_TYPES).forEach(function (type) {
...
Parser.prototype.array = function (varName, options) {
  ...
  if (typeof options.type === "string" && !aliasRegistry[options.type] && (0, _keys2.default)(PRIMITIVE_TYPES).indexOf(NAME_MAP[options.type]) < 0) {
...
Parser.prototype.choice = function (varName, options) {
  if (arguments.length == 1 && (typeof varName === "undefined" ? "undefined" : (0, _typeof3.default)(varName)) === "object") {
    options = varName;
    varName = null;
  }
  ...
  (0, _keys2.default)(options.choices).forEach(function (key) {
    ...
    if (typeof options.choices[key] === "string" && !aliasRegistry[options.choices[key]] && (0, _keys2.default)(PRIMITIVE_TYPES).indexOf(NAME_MAP[options.choices[key]]) < 0) {
...
Parser.prototype.sizeOf = function () {
  ...
  if ((0, _keys2.default)(PRIMITIVE_TYPES).indexOf(this.type) >= 0) {
...
Parser.prototype.generateAssert = function (ctx) {
  ...
  switch ((0, _typeof3.default)(this.options.assert)) {
    case "function":
...
Parser.prototype.generateBit = function (ctx) {
  ...
  var parser = JSON.parse((0, _stringify2.default)(this));
  ...
  if (!this.next || this.next && ["Bit", "Nest"].indexOf(this.next.type) < 0) {
    ...
    ctx.bitFields.forEach(function (parser) {
      ctx.pushCode("{0} = {1} >> {2} & {3};", parser.varName, val, isBigEndian ? sum - bitOffset - parser.options.length : bitOffset, (1 << parser.options.length) - 1);
...
Parser.prototype.generateString = function (ctx) {
  ...
  if (this.options.length && this.options.zeroTerminated) {
    ...
    ctx.pushCode("{0} = buffer.toString('{1}', {2}, offset - {2} < {3} ? offset - 1 : offset);", name, this.options.encoding, start, this.options.length);
  } else if (this.options.length) {
    ctx.pushCode("{0} = buffer.toString('{1}', offset, offset + {2});", name, this.options.encoding, ctx.generateOption(this.options.length));
    ctx.pushCode("offset += {0};", ctx.generateOption(this.options.length));
  } else if (this.options.zeroTerminated) {
    ctx.pushCode("var {0} = offset;", start);
    ctx.pushCode("while(buffer.readUInt8(offset++) !== 0);");
    ctx.pushCode("{0} = buffer.toString('{1}', {2}, offset - 1);", name, this.options.encoding, start);
  } else if (this.options.greedy) {
    ctx.pushCode("var {0} = offset;", start);
    ctx.pushCode("while(buffer.length > offset++);");
    ctx.pushCode("{0} = buffer.toString('{1}', {2}, offset);", name, this.options.encoding, start);
  }
...
Parser.prototype.generateChoice = function (ctx) {
  ...
  (0, _keys2.default)(this.options.choices).forEach(function (tag) {
